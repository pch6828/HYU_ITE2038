...
int open_table(char* pathname) {
	...
	if (id_table->cnt == 10) {
		...
		int idx = atoi(pathname + 4);
		if (id_table->id[idx] == -1) {
			res = open_db(pathname);
			if (res == -1) {
				return res;
			}
			id_table->cnt++;
			id_table->id[idx] = res;
			id_table->pathname[idx] = (char*)malloc(strlen(pathname) + 1);
			strcpy(id_table->pathname[idx], pathname);
		}
		return idx;
	}
}
