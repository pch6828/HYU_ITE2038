...
void abort_trx(int trx_id) {
	...
	aborted_trx.insert(trx_id);

	for (auto it = trx_table[trx_id]->trx_locks.rbegin(); it != trx_table[trx_id]->trx_locks.rend(); it++) {
		if ((*it)->undo) {
			int idx = buf_get_page((*it)->table_id, (*it)->page_no);
			page_t* page = buffer->frames[idx].page;
			for (int i = 0; i < page->header.num_keys; i++) {
				if (page->records[i].key == (*it)->undo->key) {
					memcpy(page->records[i].value, (*it)->undo->before, 120);
				}
			}
		}
	}
